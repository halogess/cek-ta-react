...
export default function MinScoreSettings({ minScore, tempMinScore, onTempScoreChange, onSave, hasChanges }) {
  return (
    <Paper elevation={0} sx={{ p: 3, borderRadius: '12px', border: '1px solid #E2E8F0' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box sx={{ flex: 1 }}>
          <Typography variant="h5" fontWeight="bold">Pengaturan Validasi</Typography>
          <Typography color="text.secondary" variant="body2">Atur minimal skor kelulusan validasi dokumen</Typography>
        </Box>
        <Stack direction="row" spacing={3} alignItems="center" sx={{ minWidth: 400 }}>
          ...
          <Button variant="contained" startIcon={<SaveOutlined />} onClick={onSave} disabled={!hasChanges}>
            Update
          </Button>
        </Stack>
      </Box>
